...
--[[
 options は net event 経由でも渡ってくる (l2k_geoanim:client:*)。
 timeScale が 0 / 負 だと経過時間が進まず演出が終わらなくなり、
 autoDisableAfter が巨大だと同じく出っぱなしになるため、範囲を固定する。
]]
local function sanitizeOptionNumber(value, minValue, maxValue)
...
local function startInstall(vehicle, options)
    ...
    if vehicle and vehicle ~= 0 and DoesEntityExist(vehicle) then
        ...
        local radius = sanitizeOptionNumber(options.radius, 1.0, 200.0)
            or Config.defaultSearchRadius or 32.0
        state.targetVehicle = findClosestVehicle(coords, radius)
    end
    ...
    state.timeScale = sanitizeOptionNumber(options.timeScale, 0.05, 10.0)
        or profileConfig.timeScale or Config.timeScale or 1.0
    state.autoDisableAfter = sanitizeOptionNumber(options.autoDisableAfter, 0.1, 120.0)
        or profileConfig.autoDisableAfter or Config.autoDisableAfter
    state.elapsed = 0.0
    state.enabled = true
